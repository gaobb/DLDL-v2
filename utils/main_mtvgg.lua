...
local Trainer = require 'train_mtvgg'
...
local savePath = paths.concat(opt.dataset, 'max2_avg7', opt.loss..'_'..opt.netType..'_CR'..opt.CR)
--local savePath = paths.concat(opt.dataset, 'avg14', opt.loss..'_'..opt.netType..'_CR'..opt.CR)
if opt.dataAug == 'true' then
   if opt.labelStep ==1 or opt.labelStep ==0.1 then
   opt.save = savePath..'_Aug'..'_Lambda_'..opt.lambda
   else 
   opt.save = savePath..'_Aug'..'_Step_'..opt.labelStep..'_Lambda_'..opt.lambda
   end
else
   opt.save = savePath..'_Lambda_'..opt.lambda
end
print(opt.save)
